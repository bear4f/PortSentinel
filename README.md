...
IP Group 适合让多个端口共用同一批白名单。进入 `[2] IP Group 管理`，先选择 `[2] 创建 Group`，再选择 `[5] 批量添加 IP/CIDR`。一行可以输入多个地址，使用英文逗号分隔，并且可以混合 IPv4、IPv6 和 CIDR：

```text
请输入 IP Group 名称：office
请输入要添加的 IP/CIDR（多个用逗号分隔，可混合 IPv4 和 IPv6）：8.212.49.31, 192.0.2.0/24, 2001:db8::10
[信息] 已识别 IPv4 2 个、IPv6 1 个来源。
[信息] 已批量更新 IP Group：office。请返回主菜单选择 [6] 应用配置。
```

重复添加的地址会自动去重。批量输入中只要有一个地址无效，面板就会要求重新输入整组地址，不会保存一半。修改 Group 后，所有绑定它的端口都会在下次应用配置时使用更新后的白名单。
